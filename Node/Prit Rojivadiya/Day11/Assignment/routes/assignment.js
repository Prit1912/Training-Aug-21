...
router1.get('/',(req,res)=>{
    empAssignments.find({addressline2: /^guj/i},(err,docs)=>{
        if(docs.length == 0){
            res.status(404).send('not found');
            return;
        }
        res.send(docs);
    }).or([{empId: 1},{addressline1: "Surat"}])
})
...
router1.get('/:empid/child/assignments',(req,res)=>{
    empAssignments.find({empId: req.params.empid,addressline2: /rat$/i},(err,doc)=>{
        if(doc.length == 0){
            res.status(404).send('not found');
            return;
        }
...
router1.post('/:empid/child/assignments',verifyToken,(req,res)=>{
